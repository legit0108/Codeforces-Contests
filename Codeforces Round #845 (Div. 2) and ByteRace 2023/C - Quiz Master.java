...
public class Main {
  ...
  static void solve() {
    ...
    while(end<n){
      while(end<n && map.size()<m){
        int val = a[end];

        for(int factor: factors[val]){
            if(factor<=m){
              int freq = map.getOrDefault(factor, 0);
              map.put(factor, freq+1);
            }  
        }
      ...
      while(map.size()==m){
        ans = Math.min(ans, a[end-1]-a[start]);
          
        int val = a[start];

        for(int factor: factors[val]){
            if(factor<=m){ 
              int freq = map.get(factor);
              freq--;
              map.put(factor, freq);
                
              if(freq==0) map.remove(factor);
            }
        }
