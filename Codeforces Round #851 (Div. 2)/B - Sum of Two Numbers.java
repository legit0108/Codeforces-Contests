...
public class Main {
  ...
  /*
   When you divide a digit into two, the difference between the two divided digits is at most one
   Divide each digit of x and y such that on summing up the digits, the difference is <= 1
   In case of an even digit, divide the digit into two halves, alternate placing the larger number in x and y when the digit is odd (in such a case, we can divide the digit as digit/2, 1+digit/2)
   This is done to avoid getting a difference of more than one between the sum of digits
  ...
  */
  ...
  static void solve() {
    ...
    int flag = 0; // To alternate placing the larger of two digits in case of odd digit
    int len = num.length();
    ...
    while(start_x<len_x-1 && x.charAt(start_x)=='0') start_x++;
    while(start_y<len_y-1 && y.charAt(start_y)=='0') start_y++;
    
    sb.append(x.substring(start_x, len_x)).append(" ");
    sb.append(y.substring(start_y, len_y)).append("\n");
  }
