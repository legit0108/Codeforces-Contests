...
public class Main {
  ...
  // precompute damage incurred for all pairs of indices
  ...
  static void solve() throws Exception{
    ...
    for(int prefIdx = 1;prefIdx<=n;prefIdx++){
        if((arr[prefIdx]-arr[prefIdx-1])<0) damagePref+=arr[prefIdx-1]-arr[prefIdx];
        pref[prefIdx] = damagePref;
        
        int suffIdx = n-prefIdx+1;
        
        if(suffIdx+1<=n&&(arr[suffIdx]-arr[suffIdx+1])<0) damageSuff+=arr[suffIdx+1]-arr[suffIdx];
        suff[suffIdx] = damageSuff;
    }
    ...
    while(m-->0){
        ...
        if(sj<tj){
            sb.append(pref[tj]-pref[sj]).append("\n");
        }else{
            sb.append(suff[tj]-suff[sj]).append("\n");
        }
    }
  }
