...
public class Main {
  ...
  static void solve() throws Exception{
    int nodes = i();
    long self[] = new long[nodes+1];
    long sum[] = new long[nodes+1];
    ArrayList<Integer> tree[] = new ArrayList[nodes+1];
    for(int node = 1;node<=nodes;node++) tree[node] = new ArrayList();
    ...
    long left[] = new long[nodes+1];
    long right[] = new long[nodes+1];
    ...
    dfs(1,tree,self,sum,left,right);
    
    long ans = 0;
    for(int node = 1;node<=nodes;node++){
        if(self[node]>sum[node]) ans++;
    }
    
    sb.append(ans).append("\n");
  }
  
  private static void dfs(int node,ArrayList<Integer> tree[],long self[],long sum[],long left[],long right[]){
      for(int child : tree[node]){
          dfs(child,tree,self,sum,left,right);
          sum[node]+=self[child];
      }
      
      if(sum[node]<left[node]) self[node] = right[node];
      else if(sum[node]<=right[node]) self[node] = sum[node];
      else self[node] = right[node];
  }
