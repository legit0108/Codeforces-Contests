...
public class Main {
  ...
  /*
   If the number of set bits decreases after querying a bit, then that bit is present in the number
   Decrement set bits one by one
   However, if the number of set bits increases, then that bit is not present in the number, in such a case toggling happens 
   Find the actual set bit by counting the net change that occurred due to toggling
   Return when the number of bits to left becomes 0
  ...
  */
  ...
  static void solve() {
   ...
   for(int bit=0; bit<30 && bitsToLeft>0; bit++){
       int mask = 1<<bit;
       int count = query(mask);
       
       if(count==totalBits-1){
           ans|=mask;
           bitsToLeft--;
           totalBits--;
       }else{
           int setBit = bit+(count-totalBits)+1;
           ans|=1<<setBit;
           bit = setBit;
           bitsToLeft--;
           totalBits = count;
