...
// idea : the element which is not paired up
// will lie in that half of the array which has 
// odd number of elements in range of start to end of 
// said half
...
public class Main {
  ...
  static void solve() throws Exception{
    ...
    while(low<high){
        ...
        int elems = query(low,mid);
        if(elems%2==0) low = mid+1;
        else high = mid;
    }
  ...
  private static int query(int start,int end){
      ...
      for(int idx = 1;idx<=end-start+1;idx++){
          int num = i();
