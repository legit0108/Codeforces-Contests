...
public class Main {
  ...
  // when we apply given operation on some range ,
  // for that range the indices which are already tall remain tall
  // and the ones which are short already remain short
  // increasing value at some index comes with
  // same corresponding increase for neighbour indices
  // which is same as not applying given operation
  // but this comes with an edge case 
  // for k = 1 we can select single element and increase it till it
  // becomes greater than sum of neighbors 
  // we can do this alternatively for triplets 
  // so our maximum answer for k = 1 will be (n-1)/2
  ...
  static void solve() throws Exception{
    ...
    for (int idx = 0; idx < n; idx++) {
      if (idx > 0 && idx < n - 1) {
        if (arr[idx] > (arr[idx - 1] + arr[idx + 1])) count++;
      }
    }

    if(k==1) count = Math.max(count, (n -1) / 2);
