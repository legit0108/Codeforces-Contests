...
public class Main {
  ...
  /*
   Let f(x) denote the number of moves required to make the strings equal having "x" different characters
   f(x) = x/n*(1+f(x-1)) + ((n-x)/n)* (1+f(x+1))
   Simplifying: f(x) = 1/(n-(x-1)) * (n*f(x-1) -n -(x-1)*f(x-2))
   f(0) = 0
   f(1) = 2^n-1
  ...
  */
  ...
  static void solve() {
    ...
    for(int idx=0; idx<n; idx++){
        if(a.charAt(idx) != b.charAt(idx)) diff++;
    }
    
    if(diff==0) sb.append(0).append("\n");
    else if(diff==1){
        sb.append(((p(2, n)-1l)+mod)%mod).append("\n");
    }else{
        long fn_minus_two = 0;
        long fn_minus_one = (p(2l, n)-1l+mod)%mod;
        
        for(long x=2; x<=diff; x++){
            long fn = (((n*fn_minus_one%mod-(x-1)*fn_minus_two-n+mod)%mod)*p(n-(x-1), mod-2))%mod;
            fn_minus_two = fn_minus_one;
            fn_minus_one = fn;
        }
        
        if(fn_minus_one<0) fn_minus_one+=mod;
        sb.append(fn_minus_one).append("\n");
    }
  }
