...
public class Main {
  ...
  /*
   Let the number of unique characters in a be d
   Total possibilities = dCk, which is small because d and k are <=10
  ...
  */
  ...
  static void solve() {
    ...
    String uniqueStr = getUniqueStr(a, n);
    int uniqueStrLen = uniqueStr.length();
    k = Math.min(k, uniqueStrLen);
    
    generatePossibilites(a, b, uniqueStr, 0 , 0, n, k, uniqueStrLen);
    sb.append(ans).append("\n");
  } 
  
  private static void generatePossibilites(String a, String b, String uniqueStr, int idx, int mask, int n, int k, int uniqueStrLen){
    if(idx==uniqueStrLen){
        if(Integer.bitCount(mask)==k) solve(a, b, mask, n);
        return;
    }  
    
    generatePossibilites(a, b, uniqueStr, idx+1, mask, n, k, uniqueStrLen); // exclude
    
    int pos = uniqueStr.charAt(idx)-'a';
    mask|=1<<pos;
    generatePossibilites(a, b, uniqueStr, idx+1, mask, n, k, uniqueStrLen); // include
  }
  
  private static void solve(String a, String b, int mask, int n){
    long substrings = 0;
    long count = 0;
    
    for(int idx=0; idx<n; idx++){
        char ch_a = a.charAt(idx);
        char ch_b = b.charAt(idx);
        int pos = ch_a-'a';
        
        if(ch_a==ch_b || (mask&(1<<pos))!=0) count++; // contiguous segment of [l, r] continues if char_a == char_b or our set has char_a (in such a case we replace char_a by char_b)
        else{
    ...
    substrings+=count*(count+1)/2;
    ans = Math.max(ans, substrings);
  }
  
  private static String getUniqueStr(String str, int len){
  ...
  public static void main(String[] args) throws Exception{
    ...
    // factors = new ArrayList[(int)1e5+1];
    // getFactors();
