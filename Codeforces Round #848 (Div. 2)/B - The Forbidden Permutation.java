...
public class Main {
  ...
  /*
   For m-1 adjacent pair of elements, find moves required to make the condition false (moves needed to make the array good)
   Even if one adjacent pair does not satisfy the equation, the array becomes good
  ...
  */
  ...
  static void solve() {
    ...
    int m = i();
    int d = i();
    int[] perm = readArrayi(n);
    int[] arr = readArrayi(m);
    
    int[]pos = new int[n+1];
    int ans = Integer.MAX_VALUE;
    for(int idx=0; idx<n; idx++) pos[perm[idx]] = idx+1;
    
    for(int idx=0; idx<m-1; idx++){
        int pos2 = pos[arr[idx+1]];
        int pos1 = pos[arr[idx]];
        int dist = pos2 - pos1;
        
        if(dist<0 || dist>d){ // No need to perform any operations, the array is already good
            ans = 0;
            break;
        }else{
            int moves = dist; // Swap adjacent elements till arr[idx+1] is before arr[idx]
            ans = Math.min(ans, moves);
            
            int opsRequired = d+1-dist; // Operations required to make the distance between arr[idx] and arr[idx+1] >d -> d+1
            int availableOps = (pos1-1)+(n-pos2); // Number of operations we can perform on arr[idx], arr[idx+1] respectively
            
            if(opsRequired<=availableOps) ans = Math.min(ans, opsRequired);
        }
    }
