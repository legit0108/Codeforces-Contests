...
public class Main {
  ...
  // It can be proven that if minimum element occurs on first position, then it is a losing state for current player
  // TC: O(n)
  ...
  static void solve() throws Exception{
    ...
    for(long val : arr) min = Math.min(min, val);
    
    if(arr[0]==min) sb.append("Bob\n");
