...
public class Main {
  ...
  /* 
    no value in a can be present more than two times
    if any element in a is present twice, some other element will not be present
    if any element in a is present twice, some other element smaller than the one which occurs twice should not be present 
    (so that it can pair up with the one present twice)
    for all the pairs(two same numbers), the biggest number of these pairs, pair it up with the biggest number not present in a
    if not possible to pair up elements in this fashion, then it will never be possible
...
public class Main {
  ...
  /* 
    ...
    Solution-2: Simulate sorting using array
