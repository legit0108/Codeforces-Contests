...
public class Main {
  ...
  /* 
   Bitwise and operator is a decreasing operator, bitwise or is increasing operator, no such condition on bitwise xor operator
   Increase the number of operands, the result will always decrease  
   Use binary search to find m
   Binary search + Bitwise AND of Numbers Range

   
   TC: O(log(high-low)*64)
   SC: O(1)
  */
  ...
  static void solve() {
    ...
    while(low<=high){
        ...
        long rangeBitwiseAnd = getRangeBitwiseAnd(n, mid);
        
        if(rangeBitwiseAnd<x) high = mid-1;
        else if(rangeBitwiseAnd==x){
            ans = mid;
            high = mid-1;
        }else low = mid+1;
    }
  ...
  private static long getRangeBitwiseAnd(long left, long right) {
      long ans = 0;
      long ones = 1;
