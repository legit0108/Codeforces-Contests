...
public class Main {
  ...
  static void solve() {
    ...
    while(n-->0){
        ...
        while(ki-->0){
            long pos = l();
            ans.put(pos, ans.getOrDefault(pos, 0l)+1l);
            list.add(pos);
        }
    ...
    for(long key: ans.keySet()){
        temp.put(key, temp.getOrDefault(key, 0l)+ans.get(key));
    }
    
    for(List<Long> list: input){
        for(long pos : list){
            temp.put(pos, temp.get(pos)-1l);
        }
        
        boolean possible = true;
        
        for(long key : list){
            long tempFreq = temp.get(key);
            long ansFreq = ans.get(key);
            
            if(tempFreq==0 && ansFreq!=0){
                possible = false;
                break;
            }
        }
