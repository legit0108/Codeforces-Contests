...
public class Main {
  ...
  /*
    Type 1 operation: 
    -> If on the nth day, snail reaches the top, total distance travelled in n-1 days = (n-1)*a - (n-2)*b (say min)
       (This value will be less than the height of the tree)
    -> Total distance travelled in n days = n*a - (n-1)*b (say max)
    -> min < Height of tree <= max
    
    Type 2 operation:
    -> If we know min and max (from previous type 1 queries), we know the range of the height of the tree, we can determine the time taken to climb the tree
    -> Else we cannot determine the time taken to climb the tree 

    When to ignore some information?
    When the current range of height of the tree does not fit in bounds found so far 
    in such a case ignore provided information,
    else update the range for the tree (low = max(low, currLow), high = min(high, currHigh))
  */
  ...
  static void solve() throws Exception{
    ...
    while(q-->0){
      ...
      if(type==1){
         ...
         long currLow = (n-1)*a - (n-2)*b + 1;
         long currHigh = n*a - (n-1)*b;

         if(n==1) currLow = 1;

         if(currHigh<low || currLow>high) sb.append("0 ");
         else{
         ...
         long time1 = Math.max(1, ceil(low-a, a-b)+1);
         long time2 = Math.max(1, ceil(high-a, a-b)+1);

         if(time1==time2) sb.append(time1).append(" ");
         else sb.append("-1 ");
      }
    }
