...
public class Main {
  ...
  /*
   Ideas: 
   -> Sort the array
   -> Processing elements in sorted order: makes it easier to find cost
      (For an index i to be the last element, all elements from [1...i-1] should be present,
      All elements from i+1 to n should be removed)
   -> In the final permutation some element will be the last element, 
      so it is optimal to find the cost over all elements, 
      considering every element as the last element
  ...
  */
  ...
  static void solve() throws Exception{
    ...
    long minCost = c*(long)n+d; // to make the permutation [1]
    long prev = 0;
    long costSoFar = 0;
    ...
    for(int i=0; i<n; i++){
      ...
      if(val==prev){
        costSoFar+=c; // delete val because duplicates are not allowed
      }else{
        long currCost = costSoFar;

        if(val!=prev+1){
           long count = val-prev-1; // count of elements to be added
           currCost+=count*d;
           costSoFar = currCost; 
        }

        currCost+=(long)(n-i-1)*c; // delete all elements to the right and make val the last element
