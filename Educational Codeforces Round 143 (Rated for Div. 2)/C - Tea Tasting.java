...
public class Main {
  ...
  /* 
   ...
   -> After the first step, no one drinks the last tea
   -> After the second step, no one drinks the second last tea, and so on
   -> For any tea at i, it is drunk by i, i+1, i+2, .... n people
   -> Rather than finding which person drinks which tea, find out how ith tea is distributed from i...n, do summation for all teas
   -> pref[n] - pref[i-1] <= a[i] -> Everyone gets required amount of tea
   -> If this is not the case, find max j such that pref[j] - pref[i-1] <= a[i] (binary search)
   -> Remaining tea: a[i] - (pref[j]-pref[i-1]), drunk by (j+1)th person
   -> Need to add b[i] from i...j (line sweep)
  ...
  */
  ...
  static void solve() {
   ...
   for(int i = n-1; i>=0; i--){
     long total = a[i];
     int j = binarySearch(pref, i, n-1, total); // Binary search to find max j such that pref[j]-pref[i-1] <= a[i]
     long consumed = 0;
     long left = 0;

     if(j>=i){
        consumed = pref[j] - (i>0?pref[i-1]:0);
        amount[i]++;
     ...
     left = total-consumed;
     if(j+1<n) extra[j+1]+=Math.min(left, b[j+1]); // (j+1)th person drinks amount of tea left after distributing tea uniformly from i...j
   }
   ...
   for(int i =0; i<n; i++) sb.append(amount[i]).append(" ");
   sb.append("\n");
  }

  private static int binarySearch(long[] pref, int low, int high, long amount){
    int j = low-1;
    long prefixSum = (low>0?pref[low-1]:0);
   
    while(low<=high){
      int mid = low + (high-low)/2;
      
      if(pref[mid]-prefixSum<=amount){
        j = mid;
        low = mid+1;
      }else high = mid-1;
    }
