...
public class Main {
  ...
  /* 
    Move all the blocks to the left tower, and then check that there is no more than one pair of adjacent blocks of the same color. 
    If there are no such pairs, then we can divide the tower into two in an arbitrary way, and if there is exactly one pair, then we need to make a "cut" exactly between two blocks of the same color. 
    Otherwise, there will always be a pair of adjacent blocks of the same color in one of the towers.
  ...
  */
  ...
  static void solve() {
    ...
    for(int idx=1; idx<n; idx++){
      if(s.charAt(idx)==s.charAt(idx-1)) pairs++;
    }

    for(int idx=m-1; idx>=0; idx--){
      if(idx==m-1){
        if(t.charAt(idx)==s.charAt(n-1)) pairs++; 
      }else{
        if(t.charAt(idx)==t.charAt(idx+1)) pairs++;
      }
    }

    if(pairs<=1) sb.append("Yes\n");
    else sb.append("No\n");
  }
