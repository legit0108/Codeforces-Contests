...
public class Main {
  ...
  /*
   ...
   -> Swap is only beneficial for scenarios like 10
   -> For scenarios like 100000, it is not beneficial since we would need to swap again and again, it's better to delete 1 here
   -> Only 1 swap is optimal
   -> Simply delete if more than 1 swap is required
   -> Scenarios: 
      -> Delete all ones to the left, and all ones or zeros to the right (empty string or 0000 or 00001111 all are sorted)
      -> Or perform the swap, then perform the above-mentioned operation
  ...
  */
  ...
  static void solve() {
    ...
    long swapCost = (long)1e12;
    long deleteCost = (long)1e12+1;
    long minCost = deleteCost*Math.min(zeros, ones);
    ...
    for(int idx=0; idx<len; idx++){
      ...
      if(idx+1<len && str.charAt(idx)=='1' && str.charAt(idx+1)=='0'){ // swap and delete to sort
         currCost = swapCost+(prefixOnes-1l)*deleteCost + Math.min(zeros-prefixZeros-1, ones-prefixOnes+1)*deleteCost;
      }else currCost = prefixOnes*deleteCost + Math.min(zeros-prefixZeros, ones-prefixOnes)*deleteCost; // delete to sort
