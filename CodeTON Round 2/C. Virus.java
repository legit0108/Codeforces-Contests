...
public class Main {
  ...
  // greedily try to save longest chains of houses
  ...
  static void solve() throws Exception{
    ...
    long count = 0;
    int size = 0;
    long infected[] = new long[m];
    List<Long> safeLanes = new ArrayList();
    
    for(int idx=0;idx<m;idx++) infected[idx] = l();
    
    infected = sort(infected);
    
    for(int idx=1;idx<m;idx++){
       safeLanes.add(infected[idx]-infected[idx-1]-1l);
    }

    safeLanes.add((n-infected[m-1])+(infected[0]-1l));
    Collections.sort(safeLanes,Collections.reverseOrder());
    size = safeLanes.size();

    for(int idx=0;idx<size;idx++){
        if(safeLanes.get(idx)>4l*count){
          safeCount++; // secure one safe house 
          long factor = 4l*count+2; // houses infected so far
          
          safeCount+=Math.max(0,safeLanes.get(idx)-factor); // secure safe houses (if any) 
          count++;
        }else break;
    } 

    sb.append(n-safeCount).append("\n"); // infected = total - safe
  }
