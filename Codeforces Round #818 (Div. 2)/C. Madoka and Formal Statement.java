...
public class Main {
  ...
  /*
   ...
   a[idx] > b[idx] then ans is not possible
   a[idx] = b[idx] then no need to perform operation
   if a[idx] < b[idx]: 
   let's assume we can make a[(idx + 1)%n] = b[(idx + 1)%n]
   now, a[idx] <= a[(idx + 1)%n] implies a[idx] <= b[(idx + 1)%n] 
   a[idx] can be made at max b[(idx + 1)%n] + 1
   since we have to make a[idx] = b[idx]
   b[idx] <= b[(idx + 1)%n] + 1
   if this is true for all idx, then ans is possible
   cyclic dependency always allows ans to be possible
   so we check cyclically
