...
public class Main {
  ...
  // The distance we travel is directly related to 
  // maximum amount we need to travel to
  ...
  static void solve() throws Exception{
    ...
    for(int idx =0;idx<n;idx++){
      ...
      if(arr[idx][0]==0){
          if(arr[idx][1]>0) max1 = Math.max(max1,Math.abs(arr[idx][1]));
          else max2 = Math.max(max2,Math.abs(arr[idx][1]));
      }else if(arr[idx][1]==0){
          if(arr[idx][0]>0) max3 = Math.max(max3,Math.abs(arr[idx][0]));
          else max4 = Math.max(max4,Math.abs(arr[idx][0]));
      }
    }
    
    long ans = (max1+max2+max3+max4);
